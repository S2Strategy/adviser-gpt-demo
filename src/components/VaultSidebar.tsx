...
export function VaultSidebar() {
  ...
  return (
    <div
      className={`h-full flex flex-col py-4 transition-all duration-300 ${isCollapsed ? 'w-[60px]' : 'w-[300px]'
        }`}
    >
        ...
        {isCollapsed ? (
          ...
          <>
            <div className="text-sidebar-foreground font-semibold text-sm">
              <Logo aria-label="Orrigo" className="h-6 w-auto" />
            </div>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="sm" onClick={toggleSidebar}>
                ...
                <div className="pl-5">
                  ...
                  >
                    <div className="flex items-center justify-center w-4 h-4 relative">
                      <ChevronDown
                        className={`w-4 h-4 absolute transition-all duration-300 ${!isChatsExpanded
